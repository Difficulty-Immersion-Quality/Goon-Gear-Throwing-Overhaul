-- ==================================== BENGT EDITION RETURNING WEAPON CODE ====================================
-- TODO: Test functionality on NPCs.

local RETURNING_ITEM_TAG = "b5b9303d-a3cb-435b-b8d6-a5560cd48815"

-- Equipment inventory slots from InventoryMember.EquipmentSlot.
local MELEE_MAIN_HAND = 3
local MELEE_OFF_HAND = 4
...
Ext.Entity.OnCreateDeferred("SpellCastTargetsChangedEvent", function(spellCastEntity)
    ...
    -- for non-throwing spell previews.
    if selectedTarget == nil or selectedTarget.TargetingType ~= "Throw" then
        return
    end
    ...
    pendingThrowsByCharacter[casterUuid] = {
        ...
        offHandItemUuid = offHandSlot
            and offHandSlot.Item.Uuid.EntityUuid
    }
end)

Ext.Osiris.RegisterListener("OnThrown", 7, "after",
    function(thrownItemUuid, _itemTemplate, throwerUuid)
        local thrownItemEntity = Ext.Entity.Get(thrownItemUuid)
        local throwerEntity = Ext.Entity.Get(throwerUuid)
        local hasReturningItemTag = false

        for _, tagUuid in pairs(thrownItemEntity.Tag.Tags) do
            if tagUuid == RETURNING_ITEM_TAG then
                hasReturningItemTag = true
                break
            end
        end

        local throwerEntityUuid = throwerEntity.Uuid.EntityUuid
        local handSnapshot = pendingThrowsByCharacter[throwerEntityUuid]
        pendingThrowsByCharacter[throwerEntityUuid] = nil

        if not hasReturningItemTag then
            return
        end
        ...
        local currentMainHandUuid = currentMainHandSlot
            and currentMainHandSlot.Item.Uuid.EntityUuid
        ...
        if handSnapshot.equipmentSlot == MELEE_MAIN_HAND then
            ...
            -- it might move the old offhand weapon into the empty main hand, it did once in testing idk if it always does that but w/e if it does, it works, if it doesn't, it still works.
            if currentMainHandUuid == handSnapshot.offHandItemUuid
                and currentOffHandUuid == nil then
                Osi.Unequip(throwerUuid, currentMainHandUuid)
                Osi.Equip(throwerUuid, thrownItemUuid, 1, 1, 0)
                Osi.Equip(throwerUuid, currentMainHandUuid, 1, 1, 0)
                return
            end
...
-- ==================================== OG RETURNING WEAPON CODE HEAVILY REFACTORED ====================================
